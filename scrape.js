...
const scrape = async () => {
  ...
  const url = 'you-url-here-with-hee';
  await page.goto(url);

  const books = await page.evaluate(() => {
    const bookElements = document.querySelectorAll('.service-item');
    return Array.from(bookElements).map((book) => {
      const title = book.querySelector('h6 strong').innerHTML;
      const desc = book.querySelector('.hight-p').innerHTML
      return {
        title,
        desc
      };
